...
{
  SphericalCoords sc;
  sc.longitude = static_cast<float>(ssP.x * M_PI);
  sc.latitude  = static_cast<float>(ssP.y * (M_PI * 0.5));
  sc.radius    = static_cast<float>(ssP.z);
...
SphericalCoords SphericalCamera::cartToSphere(const Vector &cc) const
{
  ...
  sc.radius = static_cast<float>(std::sqrt(cc.x * cc.x + cc.y * cc.y + cc.z * cc.z));

  if (sc.radius == 0.0) {
    return SphericalCoords();
  }

  sc.longitude = static_cast<float>(atan2(cc.x, cc.z));
  sc.latitude  = static_cast<float>(M_PI * 0.5 - acos(cc.y / sc.radius));
...
Vector SphericalCamera::sphereToCart(const SphericalCoords &sc) const
{
  ...
  const float theta = static_cast<float>(0.5 * M_PI - sc.latitude);
